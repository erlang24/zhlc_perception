...
# GroundSegmentationPipeline实现了完整的地面分割pipeline：
#     初始化配置参数
#     获取车辆信息
#     创建处理管道
#     点云过滤和拼接
...
# 输入: /sensing/lidar/concatenated/pointcloud
# 输出: /perception/obstacle_segmentation/pointcloud

# 输入点云 → 裁剪框过滤 → 地面分割 → 单帧原始点云 → 时序过滤 → 最终输出点云
    ...
    def create_additional_pipeline(self, lidar_name):
        ...
        components.append(
            ComposableNode(
                ...
                name=f"{lidar_name}_crop_box_filter", # 节点的名称
                remappings=[
    ...
    def create_time_series_outlier_filter_components(input_topic, output_topic):
        ...
        components.append(
            ComposableNode(
                ...
                name="occupancy_grid_map_outlier_filter", #占据网格地图异常值滤波器
                remappings=[
...
def launch_setup(context, *args, **kwargs):
    pipeline = GroundSegmentationPipeline(context) # 创建pipeline实例
    ...
    components.extend(
        pipeline.create_single_frame_obstacle_segmentation_components(
            ...
            output_topic=pipeline.single_frame_obstacle_seg_output  # 实际输出话题：/perception/obstacle_segmentation/single_frame/pointcloud_raw
...
def generate_launch_description(): # 入口函数
    launch_arguments = []
