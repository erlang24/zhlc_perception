...
class PointcloudMapFilterPipeline:
    ...
    def create_pipeline(self):
        if self.use_pointcloud_map == "true":
            return self.create_compare_map_pipeline() # true
        else:
    ...
    def create_compare_map_pipeline(self):  #this
        components = []
        down_sample_topic = (
                ...
                parameters=[
                    {
                        ...
                        "downsize_ratio_z_axis": self.downsize_ratio_z_axis, #Z轴下采样比例
                        "timer_interval_ms": self.timer_interval_ms, #定时器间隔
                        "use_dynamic_map_loading": self.use_dynamic_map_loading, # 是否使用动态地图加载
                        "map_update_distance_threshold": self.map_update_distance_threshold, #地图更新距离阈值
                        "map_loader_radius": self.map_loader_radius, #地图加载器半径
                        "publish_debug_pcd": self.publish_debug_pcd, #是否发布调试点云
                        "input_frame": "map", #输入坐标系，设为"map"
                    }
                ],
                extra_arguments=[
