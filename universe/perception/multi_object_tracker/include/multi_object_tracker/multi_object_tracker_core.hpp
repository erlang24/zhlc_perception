...
private:
  ...
  std::map<std::uint8_t, std::string> tracker_map_; // 跟踪器类型映射
  ...
  std::list<std::shared_ptr<Tracker>> list_tracker_; // 跟踪器列表
  std::unique_ptr<DataAssociation> data_association_; // 数据关联器

  void checkTrackerLifeCycle(
    std::list<std::shared_ptr<Tracker>> & list_tracker, const rclcpp::Time & time,
    const geometry_msgs::msg::Transform & self_transform);
  void sanitizeTracker(
    std::list<std::shared_ptr<Tracker>> & list_tracker, const rclcpp::Time & time); // 清理无效的跟踪器
  std::shared_ptr<Tracker> createNewTracker(
    const autoware_auto_perception_msgs::msg::DetectedObject & object, const rclcpp::Time & time, 
    const geometry_msgs::msg::Transform & self_transform) const; // 创建新的跟踪器实例
