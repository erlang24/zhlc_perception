...
{ 
  auto it = traffic_light_to_element_map.find(traffic_light.id());
  if (it != traffic_light_to_element_map.end()) {
      roi.traffic_light_id = it->second;
  } else {
  ...
  const autoware_planning_msgs::msg::LaneletRoute::ConstSharedPtr input_msg)
{
  ...
  for (auto tl_itr = route_lanelet_traffic_lights.begin();
    ...
    for (auto lsp : lights) {
      ...
      traffic_light_to_element_map[lsp.id()] = tl->id();
